...
""", unsafe_allow_html=True)
    ...
    if open_df.empty:
        ...
        for _, row in open_df.iterrows():
            ...
            if entry_date:
                # rec_entry_zone is NaN (a truthy float) for trades without a rec
                rec_zone = getattr(row, "rec_entry_zone", None)
                rec_zone = rec_zone if isinstance(rec_zone, str) else ""
